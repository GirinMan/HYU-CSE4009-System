...
void eval(char *cmdline) 
{
    ...
    if(!builtin_cmd(argv)){
        ...
        // Job is run by the child process
        if((pid = fork()) == 0){
            
            setpgid(0, 0);
            sigprocmask(SIG_UNBLOCK, &set, NULL);

            // execve returns only when error has occured
        ...
        if (!bg){ /* Foreground job */
            ...
            // Parent wait until the child process terminates
            waitfg(pid);
            
        }
        else{ /* Background job */
            // Print the information of current job
